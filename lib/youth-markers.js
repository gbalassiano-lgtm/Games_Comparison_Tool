...
// allowShortTierLabel: when false, a bare "<word> <digit>" label is not treated as a
// league tier. Callers that already know the text is a fixture team name pass false so
// club reserves ("Zamora 2") keep their marker.
function isLeagueTierCompetitionLabel(text = '', options = {}) {
  const allowShortTierLabel = options.allowShortTierLabel !== false;
  const normalized = stripCompetitionPhaseSuffix(
    String(text || '').toLowerCase().replace(/\u00a0/g, ' ')
  );
...
// knownTeam: text came from a fixture team field, so a bare "<club> <digit>" name is a
// reserve side and never a league tier.
function extractSideCategoryMarkers(text = '', options = {}) {
  const role = options.role === 'competition' ? 'competition' : 'team';
  const knownTeam = role === 'team' && options.knownTeam === true;
  ...
  if (youthMatch) {
    ...
    if (
      /\b(reserves?|reservas?|res|amateure|ama)\b/.test(raw) ||
      /\s+am$/.test(trimmed) ||
      (
        /\s+(ii|2|b)$/.test(trimmed) &&
        !isLeagueDivisionBLabel(raw) &&
        !isLeagueTierCompetitionLabel(raw, { allowShortTierLabel: !knownTeam })
      )
    ) {
      youthKey = 'reserve';
    }
...
function aggregateCategoryMarkers(texts = [], options = {}) {
  const role = options.role === 'competition' ? 'competition' : 'team';
  const knownTeam = options.knownTeam === true;
  const acc = { women: false, men: false, youthKeys: new Set() };
  for (const text of texts || []) {
    if (!text) continue;
    mergeCategoryMarkerInto(acc, extractSideCategoryMarkers(text, { role, knownTeam }));
  }
  return acc;
}
...
function aggregateFixtureCategoryMarkers(texts = []) {
  ...
  if (list.length >= 3) {
    ...
    const acc = aggregateCategoryMarkers(teams, { role: 'team', knownTeam: true });
