...
function tomorrowIsoInTimezone(timezone = SCAN_TIMEZONE) {
  const now = new Date();
...
function formatDateKeyInTimezone(startTime, timezone = SCAN_TIMEZONE) {
...
function resolveScanTargetDate(rawDate, timezone = SCAN_TIMEZONE) {
  const value = String(
...
function todayIsoInTimezone(timezone = SCAN_TIMEZONE) {
  return new Intl.DateTimeFormat('en-CA', {
    timeZone: timezone,
    year: 'numeric',
    month: '2-digit',
