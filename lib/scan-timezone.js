...
function isStaleFinishedGameStatus(status = '') {
  ...
  return [
    ...
    'suspended', 'abandoned', 'interrupted', 'walkover', 'retired',
  ].includes(s);
}

/** True when dateKey+time (in timezone) is still after `now`. */
function isUpcomingKickoff(dateKey, time, {
  now = new Date(),
  timezone = resolveScanTimezone(),
} = {}) {
  if (!dateKey || !time) return false;
  const kickoff = localDateTimeInZoneToUtc(String(dateKey), String(time), timezone);
  if (!kickoff || Number.isNaN(kickoff.getTime())) return false;
  return kickoff.getTime() > now.getTime();
}

/**
 * Drop finished/live rows unless kickoff is still in the future
 * (365Scores sometimes marks exhibition/UTS fixtures IsFinished before start).
 */
function shouldDropStaleFinishedGame(status, dateKey, time, options = {}) {
  if (!isStaleFinishedGameStatus(status)) return false;
  return !isUpcomingKickoff(dateKey, time, options);
}
