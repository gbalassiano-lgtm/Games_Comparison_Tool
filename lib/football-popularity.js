...
function lookupPriorityEntry({ competition = '', country = '' } = {}) {
  ...
  // When country is known but did not match, do not guess from name alone (e.g. Guatemala
  // "Primera Division" must not inherit Spain LaLiga's alias rank).
  if (countryKey) return null;

  // Fall back to unique name-only matches when country is unknown.
  const nameOnly = [];
  for (const nameKey of nameKeys) {
