...
function normalizeReserveTeamSuffix(text = '') {
  return String(text || '')
    .replace(/\b(akademia|akatemia|academy|reserves?|reservas?)\b/g, ' 2')
...
function normalizeTeamNameCore(text = '') {
  let value = canonicalizeRomanNumerals(
    normalizeWomenTeamSuffix(String(text || ''))
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase()
  );
  ...
  value = value
    .replace(/\b(junior|juniors|reserves?|res|reservas?|motors?|motor|force)\b/g, ' ')
    .replace(/\b\d{3,}\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
function flexibleNameSimilarity(a = '', b = '') {
  ...
  const initialism = Math.max(initialismScore(a, b), initialismScore(b, a));
  ...
  const dice = nameDice(left, right);
  if (dice >= 0.72) return Math.max(0.9, dice);
