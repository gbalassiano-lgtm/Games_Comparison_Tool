...
const TRANSLITERATION_REPLACEMENTS = [
  ...
  // Kazakhstan: 365 Qyzyljar ↔ Flash Kyzylzhar (same club, different Latinization).
  [/\bqyzyljar\b/g, 'kyzylzhar'],
  [/\bkizil(?:zhar|jar)\b/g, 'kyzylzhar'],
  [/\bkyzylzhar\b/g, 'kyzylzhar'],
];
...
function normalizeReserveTeamSuffix(text = '') {
  return String(text || '')
    // "Res." / "Reserve" / "B" / "II" / "AM" (Amateure) → same "2" marker.
    .replace(/\b(akademia|akatemia|academy|reserves?|reservas?|res|amateure|ama)\b/g, ' 2')
    .replace(/\s+(ii|2|b|am)\s*$/i, ' 2')
    .replace(/\s+2\s*$/i, ' 2')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function normalizeTeamNameCore(text = '') {
  ...
  value = value
    ...
    .replace(/^(al|el)\s+/g, '')
    // ge/ec/sk/… are cosmetic club prefixes (GE Gloria, EC Guarani, SK Sturm).
    .replace(/\b(fc|cf|sc|ac|ec|ge|se|es|af|rc|sd|ud|rcd|ca|cd|fk|sk|bk|if|hk|nk|ik|pfc|nj|ny|hb)\b/g, ' ')
    .replace(/[:.!?_\-/]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
