...
async function scrapeFlashscoreBaseballOnce(options = {}) {
  ...
  const outputFile = options.outputFile || path.join(outputDir, 'flashscore_tomorrow_baseball_all_countries.json');
  ...
  try {
    logStep(`Starting Flashscore baseball scraper for ${targetDate}`);
...
async function scrapeFlashscoreBaseball(options = {}) {
  return runWithRetry(
    () => scrapeFlashscoreBaseballOnce(options),
    'Flashscore baseball scraper',
    options
  );
}

module.exports = { scrapeFlashscoreBaseball };

if (require.main === module) {
  scrapeFlashscoreBaseball()
    .then(() => {
