...
async function scrapeFlashscoreAmericanFootballOnce(options = {}) {
  ...
  const outputFile = options.outputFile || path.join(outputDir, 'flashscore_tomorrow_american-football_all_countries.json');
  ...
  try {
    logStep(`Starting Flashscore american-football scraper for ${targetDate}`);
    await optimizePageForScraping(page);
    const results = await scrapeFlashscoreDates(
      page,
      targetDate,
      'american-football',
      { url: 'https://www.flashscore.com/american-football/', sportSlug: 'american-football' },
...
async function scrapeFlashscoreAmericanFootball(options = {}) {
  return runWithRetry(
    () => scrapeFlashscoreAmericanFootballOnce(options),
    'Flashscore american football scraper',
    options
  );
}

module.exports = { scrapeFlashscoreAmericanFootball };

if (require.main === module) {
  scrapeFlashscoreAmericanFootball()
    .then(() => {
