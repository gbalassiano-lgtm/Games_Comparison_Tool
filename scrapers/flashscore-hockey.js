...
async function scrapeFlashscoreHockeyOnce(options = {}) {
  const SPORT = 'hockey';
  ...
  const outputFile = options.outputFile || path.join(outputDir, 'flashscore_tomorrow_hockey_all_countries.json');
  ...
  try {
    logStep(`Starting Flashscore hockey scraper for ${targetDate}`);
    await optimizePageForScraping(page);
    await runFlashscorePipeline(page, targetDate, 'hockey', {
    ...
    const results = await extractMatchesFromPage(page, readAllLeaguesAndMatches, 'hockey');
...
async function scrapeFlashscoreHockey(options = {}) {
  return runWithRetry(
    () => scrapeFlashscoreHockeyOnce(options),
    'Flashscore hockey scraper',
    options
  );
}

module.exports = { scrapeFlashscoreHockey };

if (require.main === module) {
  scrapeFlashscoreHockey()
    .then(() => {
