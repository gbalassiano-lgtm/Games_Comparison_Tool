...
async function scrapeFlashscoreBasketballOnce(options = {}) {
  ...
  const outputFile = options.outputFile || path.join(outputDir, 'flashscore_tomorrow_basketball_all_countries.json');
  ...
  try {
    logStep(`Starting Flashscore basketball scraper for ${targetDate}`);
...
async function scrapeFlashscoreBasketball(options = {}) {
  return runWithRetry(
    () => scrapeFlashscoreBasketballOnce(options),
    'Flashscore basketball scraper',
    options
  );
}

module.exports = { scrapeFlashscoreBasketball };

if (require.main === module) {
  scrapeFlashscoreBasketball()
    .then(() => {
